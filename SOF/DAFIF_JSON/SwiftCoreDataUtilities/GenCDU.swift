...
class GenCDU {
		...
		pc.performBackgroundTask { moc in
				...
				do {
					...
					DispatchQueue.main.async {
						log.info("Gen_CD Done Loading into CoreData")
    ...
    class func getWithAirPortId(id: String, moc: NSManagedObjectContext) -> [Gen_CD]? {
        let CDFetchRequest = NSFetchRequest<Gen_CD>(entityName: "Gen_CD")
        let predicate = NSPredicate(format: "%K = %@", #keyPath(Gen_CD.arptIdent_CD) , id)
